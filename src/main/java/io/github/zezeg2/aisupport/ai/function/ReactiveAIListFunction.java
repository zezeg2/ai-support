...
public class ReactiveAIListFunction<T, S> extends ReactiveBaseAIFunction<List<T>, S> {
    private final Class<T> wrappedType;

    public ReactiveAIListFunction(String functionName, String purpose, List<Constraint> constraints, Class<List<T>> returnType, OpenAiService service, ObjectMapper mapper, ConstructResolver resolver, ReactivePromptManager<S> promptManager, ReactiveResultValidatorChain<S> resultValidatorChain, ExceptionValidator exceptionValidator, OpenAIProperties openAIProperties, Class<T> wrappedType) {
